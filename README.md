...
You pick a mode once (default **Deep**); switch anytime in chat or via `.ai-state/tutor-settings.md`.
...
The script installs the `.ai-instructions/` instruction tree and creates entry-point files (`AGENT.md` + `AGENTS.md` by default). If your project already has its own `AGENT.md`/`AGENTS.md`/`CLAUDE.md`, it **never overwrites them** — it offers to append a small marked pointer block, or you can skip and add it manually.
...
```
your-project/
├── AGENT.md                     # Thin pointer → .ai-instructions/README.md (yours or appended to yours)
├── AGENTS.md                    # Cross-tool standard pointer
├── .ai-instructions/             # SHARED instructions (committed, team-shared)
...
The system is a **lazy-loaded router**. The entry files are tiny pointers; the router at `.ai-instructions/README.md` is ~60 lines. Individual modes load their own instructions and drill down into sub-files only when needed:

```
AGENT.md ──▶ .ai-instructions/README.md ──▶ tutor/<mode>/mode.md ──▶ sub-files on trigger
```
...
- **Instructions** (`.ai-instructions/`) are updated by the installer.
- **State** (`.ai-state/`, `~/.ai-tutor/`) is your data and is **never touched** by the installer.
...
- **Confirm**: just-in-time tutor — before a non-trivial action, asks "Do you know X, or want a 5-minute rundown?" and records results in `.ai-state/learnt-syllabus.md` so it never re-asks about the same concept.

## 🔧 Customization

- **Mode**: edit `mode:` in `.ai-state/tutor-settings.md`, or just say "switch to crash mode".
- **Syllabus**: edit `.ai-state/tutor-syllabus.md` (Deep/Crash) to set topics and authoritative sources.
- **Confirm known concepts**: entries live in `.ai-state/learnt-syllabus.md`.
...
Agent: "I've checked your progress. To build this you need:
       ...
       Open .ai-state/quizzes/validation-quiz.md and fill it in."
```
