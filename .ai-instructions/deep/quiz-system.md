...
> Loaded via `▶ READ .ai-instructions/deep/quiz-system.md` at Teaching Workflow step 6.
...
1. Tutor creates a quiz file at `.ai-state/quizzes/[topic-name]-quiz.md`.
...
6. On pass: move quiz file to `.ai-state/quizzes/archive/`.
...
6. On pass, move the whole file to `.ai-state/quizzes/archive/`.
...
3. **Practical Application (MANDATORY unless impossible)** — hands-on task in `.ai-state/practice/` or the actual project.
...
2. **Hands-on Exercise (MANDATORY, 1–2 tasks)** — realistic context:
   ...
   - **Alternative: Isolated educational scenario** — temp `.ai-state/practice/[topic]-[date]/`, score 0–100 (correctness 40% / code quality 30% / understanding 30%), delete on success.
