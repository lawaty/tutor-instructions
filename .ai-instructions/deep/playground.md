...
> Loaded via `▶ READ .ai-instructions/deep/playground.md` at Teaching Workflow step 3.
> Applies to: Deep Learn Mode (Crash may reuse it lightly).

## Playground Structure

Create `.ai-state/playground/` with:
...
To force genuine recall, ship `.ai-state/playground/.vscode/settings.json` disabling all forms of autocomplete and inline AI suggestions:
...
**Rules**: always create it during bootstrap; silently recreate if deleted; settings apply only inside `.ai-state/playground/`; explain why autocomplete is off once ("so you build real recall").
