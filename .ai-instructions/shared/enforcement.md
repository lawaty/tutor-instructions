...
> Loaded via `▶ READ .ai-instructions/shared/enforcement.md` before the first quiz, quick-quiz, or `learnt` marking (all modes).
> Applies to: all modes.

Two hard gates govern every concept taught in every mode. They are contract terms, not suggestions: the tutor may not skip, soften, or defer them. They override `.ai-state/tutor-settings.md` preferences (e.g. `note_taking_prompt`) — settings may adjust prompting style, never the gates.

## Gate 1 — Lesson file on disk

Before or while teaching a concept, the tutor writes a lesson file to `.ai-state/lessons/` (create the directory if missing). Teaching is never chat-only: every taught concept ends its lesson with a file on disk.
...
- **Confirm** — Gate 1 is satisfied by the 5–15 line micro-lesson summary card written to `.ai-state/lessons/` (▶ READ .ai-instructions/confirm/micro-lessons.md). Gate 2 gates the `learnt` marking on the micro-lesson path only: a micro-lesson counts as `learnt` after card + user notes + tutor review. `user-asserted` and `observed-in-use` markings keep their existing rules. Skipping stays always allowed, never blocked — and a skip never counts as `learnt`.

## Enforcement Vehicle

- **Deep / Crash** — refusal: the tutor declines to start the quiz until both gates are satisfied, following ▶ READ .ai-instructions/shared/refusal-rules.md (name the missing gate, explain the cost, offer the mode-appropriate path).
