...
> Loaded via `▶ READ tutor/deep/challenges/incident-simulator.md` on explicit request (after 5+ confirmed topics).
> Applies to: Deep Learn Mode (opt-in).

Create realistic on-call debugging scenarios in `.ai/incidents/` testing diagnosis, fix, and learning from production failures.

## How It Works

1. Create `.ai/incidents/incident-[NNN]-[name]/`:
...
**Incidents stay in `.ai/incidents/` permanently** — a portfolio of debugging experience.
