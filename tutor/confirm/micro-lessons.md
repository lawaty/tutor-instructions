...
> Loaded via `▶ READ tutor/confirm/micro-lessons.md` when the user doesn't know a concept in Confirm Mode.
...
- The Confirmation Protocol finds an unknown concept (▶ READ tutor/confirm/mode.md).
...
Keep it chat-based; append a 5–15 line summary card to `.ai/lessons/` for reference.
...
After a micro-lesson, write a `learnt` entry to `learnt-syllabus.md` (▶ READ tutor/confirm/learnt-syllabus.md) with source `micro-lesson`.
